...
def barplot_vector(data):
    ...
    # Show the plot
    plt.savefig('Project3_intranet/tests/results/plots/barplot_tot.png')


def create_vector():

    pos_embed = pd.read_csv("Project3_intranet/tests/results/data/positive_results.csv")
    neg_embed = pd.read_csv("Project3_intranet/tests/results/data/negative_results.csv")
    pos_bleu = pd.read_csv("Project3_intranet/tests/results/data/bleu_positive.csv")
    neg_bleu = pd.read_csv("Project3_intranet/tests/results/data/bleu_negative.csv")

    with open('Project3_intranet/tests/results/data/gpt_test_results_positive.txt') as file_pos:
        last_line = file_pos.readlines()[-1]
        last_line = last_line.split(' ')[-1]
        last_line = last_line.split('/')[0]
    pos_gpt = int(last_line)

    with open('Project3_intranet/tests/results/data/gpt_test_results_negative.txt') as file_pos:
        last_line = file_pos.readlines()[-1]
        last_line = last_line.split(' ')[-1]
        last_line = last_line.split('/')[0]
