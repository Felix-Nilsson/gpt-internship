...
#from embeddings.chatbot import get_chat_response, reset_chat
...
if st.session_state["authentication_status"]:
    ...
    status = config['credentials']['usernames'][username]['status']
    chatbot = chatbot.Chatbot(status)
    ...
    def generate_response(prompt, patients,status):
        response = chatbot.get_chat_response(prompt, patients,status)
        return response

    with bot_container:
        if user_input:
            response = generate_response(user_input, patients,status)
