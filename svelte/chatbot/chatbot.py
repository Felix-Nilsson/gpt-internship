...
# ! Do not forget to set the environment variable !
    ...
    def get_system_message(self, chat_type):
        ...
        file_path = os.path.join(parent_directory, 'prompts', 'prompts', f'prompt_{chat_type}_test.txt')
    ...
    def get_chat_response(self,query: str, chat_type: str, settings: dict, remember=True):
        """Get a response to the query from the specified chat(-type)bot.
        
        :param query: The query/prompt.
        :param chat_type: What kind of chat ('doctor', 'patient', 'intranet' or 'internet')
        :param settings: Settings for the chatbot (e.g. language level or tools, depends on chat_type)
        ...
        system_message = self.get_system_message(chat_type) # TODO TODO continue here?
